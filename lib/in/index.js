...
export default async function npmCheck(currentState) {
    ...
    const allDependenciesIncludingMissing = Object.keys({...allDependencies, ...currentState.get('missingFromPackageJson')});

    const arrayOfPackageInfo = await Promise.all(allDependenciesIncludingMissing
