...
async function detectPreferredInstaller(cwd) {
    const preferredPmResult = await preferredPM(cwd);
    return preferredPmResult && SUPPORTED_INSTALLERS.has(preferredPmResult.name) ? preferredPmResult.name : 'npm';
}

const options = {
    cwd: cli.input[0] || await packageDirectory() || process.cwd(),
...
try {
    options.installer = options.installer === 'auto' ? await detectPreferredInstaller(options.cwd) : options.installer;
    ...
    console.log(options.debug ? createCallsiteRecord(error).renderSync() : 'For more detail, add `--debug` to the command');
    process.exitCode = 1;
}
